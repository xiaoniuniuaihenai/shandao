...
@implementation ZTMXFLoanGoodCell

+(id)cellWithTableView:(UITableView *)tableView
{
    static NSString * cellStr = @"ZTMXFLoanGoodsListCell";
    ZTMXFLoanGoodCell * cell = [tableView dequeueReusableCellWithIdentifier:cellStr];
    if (!cell) {
        cell = [[ZTMXFLoanGoodCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellStr];
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        UIView *backSecondView = [UIView new];
        backSecondView.backgroundColor = [COLOR_SRT(@"#FAB179") colorWithAlphaComponent:0.06];
        [self.contentView addSubview:backSecondView];
        
        [backSecondView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.right.top.bottom.mas_equalTo(self.contentView);
        }];
...
- (void)chooseAction
{
    ...
    self.goodsInfoModel.selected = !self.goodsInfoModel.selected;
    _chooseBtn.selected = self.goodsInfoModel.selected;
    if (_clickHandle) {
        _clickHandle(self);
    }

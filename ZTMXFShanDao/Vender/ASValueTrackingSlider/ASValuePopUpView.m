...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _backgroundLayer.shadowOpacity = 0.4;
...
- (void)layoutSubviews
{
    ...
    id textString = _textLayer.string;
    CGFloat textHeight = 30.0;
    if ([textString isKindOfClass:[NSString class]]) {
        NSString *transformTextString = (NSString *)textString;
        textHeight = [transformTextString sizeWithFont:[UIFont systemFontOfSize:15] maxW:MAXFLOAT].height;
    } else if ([textString isKindOfClass:[NSAttributedString class]]) {
        NSAttributedString *transformAttributeString = (NSAttributedString *)textString;
        textHeight = [transformAttributeString size].height;
    }

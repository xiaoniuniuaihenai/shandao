...
- (void)createView
{
    ...
    NSArray *buttonTitleArray = @[@"取消",@"完成"];
    for (int i = 0; i <buttonTitleArray.count ; i++)
    {
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(0, 0, 50, kTopViewHeight);
        [button setTitle:buttonTitleArray[i] forState:UIControlStateNormal];
        if (i == 0) {
            button.left = 14;
            [button setImage:[UIImage imageNamed:@"login_delete"] forState:UIControlStateNormal];
        }else{
            button.right = kAddressPickerScreenWidth - 14;
            [button setTitleColor:[UIColor colorWithHexString:@"2BADF0"] forState:UIControlStateNormal];
        }

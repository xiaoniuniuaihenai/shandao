...
//    self.iconImageView = iconImageView;
    ...
    // 设置字体颜色
    userNameLabel.textColor = [UIColor colorWithHexString:@"222222"];
    //  设置对齐方式
...
//    dateLabel.text = @"279起/块";
    ...
    // 设置字体大小
    detailRemarkLabel.font = [UIFont systemFontOfSize:13];
    // 设置字体颜色
    detailRemarkLabel.textColor = [UIColor colorWithHexString:@"232323"];
    //  设置对齐方式
...
//    [self.iconImageView sd_setImageWithURL:[NSURL URLWithString:goodRemark.commentorAvata] placeholderImage:[UIImage imageNamed:@"b3touxiang"]];
    ...
    self.userNameLabel.text = goodRemark.userName;
    
    self.dateLabel.frame = self.goodRemarkFrame.dateLabelF;
    NSString *dateStr = [self dateStringFromLongDate:goodRemark.rateTime];
    self.dateLabel.text = [dateStr stringByAppendingString:goodRemark.auctionSku];
    
    self.detailRemarkLabel.frame = self.goodRemarkFrame.detailRemarkLabelF;
    self.detailRemarkLabel.text = goodRemark.content;
    
    self.photosView.frame = self.goodRemarkFrame.photosViewF;
    if (goodRemark.rateDetail.count > 1) {
